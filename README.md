...
| 证据约束回答 → MongoDB | 用户原话、助手答案、查询范围、实际引用来源和图片 | 形成可继续追问、可回看和可审计的会话记录，但下一轮检索范围仍以新请求显式参数为准。 |
...
12. **持久化与返回**：用户消息和助手消息分别写入 MongoDB。非流式请求直接返回完整结果；流式请求在前置处理期间发送节点进度，进入最终生成后发送 `delta`，最后发送校验后的 `final` 事件。
13. **失败隔离**：节点异常会把本轮任务标记为 `failed`，记录错误并在流式模式发送 `error` 事件。同一会话正在生成时返回 `409` 阻止删除。
...
当前离线回归测试共 `68/68` 通过，其中包含共享 BGE-M3 模型并发编码、主体向量 FLOAT16 类型契约、三种查询范围、短文档完整直读、长文档同章节邻居扩展、查询规划与 HyDE 超时降级、原问题贯穿检索与回答、历史不继承文档范围、完整用户/助手历史、跨来源去重、连续引用、无引用通识回答、默认关闭联网和 10 个 Prompt 渲染契约测试。
